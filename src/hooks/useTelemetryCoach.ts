...
const MATCH_EXCELLENT = [
  ...
  "Красиво. Мишень сейчас явно пожалела, что вышла на работу.",
  "Вот это уже разговор. Я бы таким выстрелом кофе отпраздновал.",
  "Прямо по делу. Без суеты, без героизма, просто хорошо.",
] as const;
...
const MATCH_STABLE = [
  ...
  "Нормально, живем. Чуть спокойнее нажмешь - будет вкуснее.",
  "Почти. Мышь держи мягче, она не убежит.",
  "Хорошая попытка. Осталось уговорить палец не торопиться.",
] as const;
...
const MATCH_LOW = [
  ...
  "Ты сейчас нажал так, будто мышь должна признаться во всем. Мягче.",
  "Не воюй с кнопкой. Она маленькая, но мстит сразу.",
  "Спешка опять забежала вперед тебя. Давай вернем ее назад.",
] as const;
...
const SIGHTING_ADVICE = [
  ...
  "Поправку нажал - и проверил. Без гадания на кофейной гуще.",
  "Стрелки поправок тут не для красоты. Пусть они поработают.",
  "Сделай второй пробный. Один выстрел иногда любит драму.",
] as const;
...
const CENTER_ADVICE = [
  ...
  "Все хорошо. Не трогай поправки, а то починим то, что не ломалось.",
  "Отлично, оставь как есть. Мишень уже согласна.",
  "Можно в зачет. Только без победного танца мышью.",
] as const;

const LEVEL_TACTICS: Record<number, string> = {
  2: "Мишень едет в сторону. Веди прицел вместе с ней и не дергай вверх-вниз. Она хитрая, но не волшебная.",
  3: "Здесь дальше расстояние. Смотри на ветер и нажимай кнопку без спешки. Ветер любит самоуверенных.",
};
