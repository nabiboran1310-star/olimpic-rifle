...
export const getAiCoachFeedback = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const prompt = [
      ...
      "Ответь одной короткой фразой на русском языке, как реальный тренер: спокойно, уверенно, без роботского тона.",
      "Не добавляй лишних терминов от себя. Не используй стрелковые сокращения и не объясняй термины.",
      "Не используй слова: Обучение, Туториал, Помощь, Инструкция, Урок.",
      "Не говори, что ты ИИ, модель, ассистент или API.",
      "Если режим пробный и clickX/clickY не ноль, ответь только числом и направлением: например 2 влево или 16 вверх.",
      "Не отменяй поправку из-за высокого счета. Клики важнее оценки выстрела.",
